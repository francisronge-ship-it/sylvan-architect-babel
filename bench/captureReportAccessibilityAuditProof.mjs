...
const tables = {
  Release: [{
    id: 'proof-accessibility-release',
...
const reportReceipt = createReportStarSchemaReceipt({
  plan: {
    reportDatasetId: 'proof-accessibility-report',
...
const artifacts = [{
  artifactId: 'proof-accessibility-artifact',
...
const previewReceipt = createReportPreviewReceipt({
  plan: {
    previewId: 'proof-accessibility-preview',
...
const records = REPORT_ACCESSIBILITY_CHECK_IDS.map((checkId) => ({
  ...
  checkId,
  observationLabel: `proof-observation-${checkId}`,
  auditorIdentity: 'proof-external-auditor',
  observedAt: 'proof-opaque-time',
  evidenceRef: `proof://evidence/${checkId}`,
  evidenceSha256: digest(`proof-evidence-${checkId}`),
  findingArtifacts: [],
  provenance: { proof: true }
}));
const plan = {
  auditId: 'proof-accessibility-audit',
...
const execute = () => createReportAccessibilityAuditReceipt({
  plan,
  reportReceipt,
  previewReceipt,
  records
...
if (!deterministicRepeatEqual) {
  throw new Error('report-accessibility-audit proof repetitions differ.');
}
const networkAttempts = guard.getAttempts();
if (networkAttempts.length > 0) {
  throw new Error(
    'report-accessibility-audit proof observed network attempts: '
    + networkAttempts.join(', ')
  );
}
const sourcePaths = [
  'bench/README.md',
  'bench/captureReportAccessibilityAuditProof.mjs',
  'bench/index.js',
  'bench/reportAccessibilityAudit.js',
  'tests/benchmarkReportAccessibilityAudit.test.mjs'
];
